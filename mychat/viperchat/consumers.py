...
class UserChatConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        self.room_group_name = f"chat_{self.username}_{self.scope['user']}"
        # Join room group
        await self.channel_layer.group_add(self.room_group_name, self.channel_name)
    ...
    async def receive(self, text_data):
        ...
        await save_user_profile_message(self.scope['user'], message, user)
...
@database_sync_to_async
def save_user_profile_message(author, content, receiver):
    if content == '':
        pass
    else:
